...
public class BottomBarController : MonoBehaviour
{
    ...
    private enum State
    {
        PLAYING, SPEEDED_UP, COMPLETED, INSTANT, FADE
    }
    ...
    private IEnumerator TypeText(string text)
    {
        ...
        while (state != State.COMPLETED)
        {
            ...
            yield return new WaitForSeconds(speedFactor * 0.015f);
    ...
    private void ActSpeaker(StoryScene.Sentence.Action action, bool isAnimated = true)
    {
        ...
        switch (action.actionType)
        {
            ...
            case StoryScene.Sentence.Action.Type.DISAPPEAR:
                ...
                break;
            case StoryScene.Sentence.Action.Type.INSTANT_APPEAR: // Handle INSTANT_APPEAR
                controller.Setup(action.speaker.sprites[action.spriteIndex]);
                controller.Show(action.coords, false); // No animation
                break;
        }
    ...
    {
        if (state == State.PLAYING || state == State.SPEEDED_UP)
        {
            StopTyping();
